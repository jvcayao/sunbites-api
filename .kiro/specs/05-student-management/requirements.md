...
- [ ] `EnrollmentService::seedMonthlyPayments()` — before creating a `StudentMonthlyPayment` for a month, call `BranchMonthlyAmount::resolveAmount()`; if the resolved amount is `0`, skip that month entirely (no record created)
- [ ] `PaymentController::addRange()` — same skip guard as above; if resolved amount for a month is `0`, skip it (do not create a `StudentMonthlyPayment` record); applies to the "Add Subscription Period" flow for existing students
