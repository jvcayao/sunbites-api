...
| `payment_reminder_days` | `14` | `integer` | Payment Reminder Days (Spec 11) |
| `pre_registration_expiry_days` | `30` | `integer` | Pre-Registration Expiry Days (Spec 13) |
...
- [x] Migration: `system_configurations` table with `key` (string, unique), `value` (string), `type` (enum: integer/decimal/string), `label` (string), `description` (text, nullable)
- [x] Seeder: seed `daily_meal_rate`, `credit_limit`, `loyalty_point_threshold`, `payment_reminder_days`, `pre_registration_expiry_days` with default values and labels
- [x] `SystemConfiguration` model with `getValue(key, default)` static helper backed by `Cache::rememberForever`; cache busted on `updated` model event
- [x] `SystemConfigurationController::index()` — returns all configs; admin only
- [x] `SystemConfigurationController::update(key)` — validates `value` against type; updates record; busts cache; admin only
- [x] Replace all `config('sunbites.daily_meal_rate', 135)` calls in app code with `SystemConfiguration::getValue('daily_meal_rate', 135)`
- [x] `config/sunbites.php` retains `daily_meal_rate`, `credit_limit`, `loyalty_point_threshold` as hardcoded fallback defaults only (remove the `env()` wrappers — the DB is the source of truth; config is last-resort fallback)
- [x] Feature tests: `SystemConfigurationTest` — list (admin only), update value, invalid type rejected, non-admin 403
- [x] Frontend: `types/system-configuration.ts` — `SystemConfiguration` type
- [x] Frontend: `lib/api/system-configurations.ts` — `systemConfigApi.list()`, `systemConfigApi.update(key, value)`
- [x] Frontend: `app/(kitchen)/references/system-settings/page.tsx` — admin-only; table of all settings; edit dialog per row
- [x] Frontend: `app/(kitchen)/references/system-settings/loading.tsx` — skeleton
- [x] Frontend: `components/layouts/kitchen-layout.tsx` — add "System Settings" to References nav, visible to admin only
