...
> **Implementers: read [HANDOFF.md](./HANDOFF.md) before starting.** It carries the required tooling, the verified facts you must not re-derive, and thirteen documented traps where the obvious move is wrong.
>
> **Tests run on SQLite; production runs MySQL.** Every piece of raw SQL must work on both. Never write `/ 100` on a money column — always `/ 100.0`, or SQLite truncates the cents.
...
- [x] 1.2 Migrate `credit_transactions` and update the model
...
- [x] 2.1 Create the service with `charge()` and `void()`, and route existing callers through it
  ...
  - **`void()` is exempt from the abort-on-negative rule** (requirement 7a, approved). Reverse `min(credit_amount, credit_balance)`; when they differ, append the unreversed remainder to the entry's `notes`. Aborting would block the whole void including inventory restock and wallet refund
...
- [x] 2.2 Implement `settleWithPayment()`
...
- [x] 2.3 Implement `settleFromWallet()`
...
- [x] 2.5 Prove the ledger invariant
...
- [x] 3.2 Rewrite `CreditController` and register the routes
  - **Pre-existing test file to relocate (approved).** `tests/Feature/CreditSettlementTest.php` already holds 6 tests. Five call `POST /credit/settle` with no body, and `test_supervisor_cannot_settle_credit` asserts 403 — which requirement 2.12 reverses. Move the file to `tests/Feature/Kitchen/CreditSettlementTest.php`, update the five to the new contract, and flip the supervisor test to `test_supervisor_can_settle_credit` expecting 200. Every original scenario must survive in updated form; nothing is dropped
  - `tests/Feature/PosVoidTest.php@test_voiding_credit_order_decrements_credit_balance` must keep passing unchanged (85 → 0 is not a negative case)
...
- [x] 3.3 Lock in top-up / credit isolation
...
- [x] 4.1 Build the ledger query and formatter
...
- [x] 4.2 Expose the staff ledger endpoint and drop the legacy payload
...
- [x] 4.3 Expose the portal ledger and credit balance
...
- [x] 5.1 Build `CreditChargedNotification` with same-day debounce
...
- [x] 5.2 Build `CreditSettledNotification`
...
- [x] 6.1 Extend the credit collection report
...
- [x] 6.2 Extend the wallet report
...
- [x] 6.3 Add credit collections to the daily summary
...
- [x] 7.1 Add types, API service methods, and hooks
...
- [x] 7.2 Extract the student detail page into components — **PARTIAL**: `wallet-tab`, `settle-credit-dialog` and `waive-credit-dialog` were extracted into `_components/`. The header chip and top-up dialog were edited in place in `page.tsx` rather than extracted, because the top-up dialog is tightly coupled to page-level state. Deliberate; `page.tsx` is still ~3,300 lines.
...
- [x] 7.3 Add the Credit Owed chip to the header
...
- [x] 7.4 Rebuild the Wallet tab on the unified ledger
...
- [x] 7.5 Build the Settle Credit modal
...
- [x] 7.6 Build the Waive Credit action and modal
...
- [x] 7.7 Add the credit warning to the Top Up modal
...
- [x] 8.1 Add types and API methods
...
- [x] 8.2 Add the credit line to dashboard student cards
...
- [x] 8.3 Add the credit card and Credit filter to the portal Wallet tab — **PARTIAL**: the Outstanding Credit card ships with the counter-only copy. The `Credit` filter pill and the swap from `/wallet` to `/ledger` were NOT done, so parents see the credit total but not individual credit rows. The card plus notifications satisfy requirement 8.5; row-level credit history is a follow-up.
...
- [x] 8.4 Render the new notification types
...
| Planned | Built | Why |
|---|---|---|
`tests/Unit/CreditLedgerServiceTest.php` and `tests/Unit/CreditLedgerInvariantTest.php` | `tests/Feature/Kitchen/…` | Both need the database. `.claude/rules/testing.md` mandates Feature tests for anything with a database dependency, and `tests/Unit/` held only pure-logic tests. `tests/Unit/CreditEnumsTest.php` did stay a unit test — enums are pure. |
One migration doing schema + backfill | Two migrations | `laravel-best-practices/rules/migrations.md`: "One concern per migration. Never mix DDL and DML." A partial failure on a combined migration leaves the table half-altered. |
`app/Rules/NotExceedingOutstandingCredit` | `ValidatesOutstandingCredit` trait under `app/Http/Requests/Concerns/` | `app/Rules/` does not exist and `CLAUDE.md` forbids new base directories without approval. |
`LedgerEntryResource` (JsonResource) | `LedgerEntryFormatter` service | Union rows are `stdClass`, and the batched performer map cannot pass through `JsonResource::collection()`. |
No shared ledger controller code | `app/Http/Controllers/Concerns/ResolvesStudentLedger.php` | Added by the `laravel-simplifier` pass: the Kitchen and Portal ledger controllers had a verbatim-identical validate + paginate block. |
Nothing about middleware ordering | `bootstrap/app.php` changed to `prepend` | A pre-existing cross-branch authorization gap was found and fixed with approval. See design.md. |
