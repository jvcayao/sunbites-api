...
13. **Frontend role-gating and the same-day disabled-button hint are UX only.** The route's `role:admin|manager` middleware and the controller's self-void/time-window checks are the actual authorization boundary. Do not treat "the button renders" or "the button is enabled" as evidence the request will succeed when writing backend tests — test the endpoint directly, independent of what the frontend would have shown.
